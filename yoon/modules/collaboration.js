...
module.exports = {
	...
	leave: function (socket, msg) {
		...
		console.log("-----------------------------");
		console.log("leave-currentRoom", this.currentRoom);
		this.users.removeByValue(msg.username);
		delete this.currentRoom[socket.id];
		
		console.log("leave에서 this.users", this.users);
	...
	msg: function (io, socket, msg) {
		...
		socket.broadcast.to(this.currentRoom[socket]).emit("communication_message", chatting_message);
		socket.emit("communication_message", chatting_message);
	},
	
	//사용자의 편집 내용을 처리합니다.
	editing: function (socket, msg) {
		socket.broadcast.to(this.currentRoom[socket.id]).emit("editing_message", JSON.stringify(msg));
	}
