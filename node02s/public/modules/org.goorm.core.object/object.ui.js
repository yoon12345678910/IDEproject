...
org.goorm.core.object.ui.prototype = {
	init: function (target, parent, type, shape_name, option, callback) {
		...
		this.context_menu.init("configs/menu/org.goorm.core.object/object.ui.html", "object.ui", "", this.properties.timestamp, "", function () {
			var object_ui = $("div[id='object.ui_" + self.properties.timestamp + "']");

			//Set context menu action
			object_ui.find("a[action=cut_object]").click(function () {
				self.cut();
			});
			object_ui.find("a[action=copy_object]").click(function () {
				self.copy();
			});
			object_ui.find("a[action=paste_object]").click(function () {
				self.paste();
			});
			object_ui.find("a[action=delete_object]").click(function () {
			...
			object_ui.find("a[action=bring_to_front]").click(function () {
				self.bring_to_front();
			});
			object_ui.find("a[action=send_to_back]").click(function () {
				self.send_to_back();
			});
			object_ui.find("a[action=bring_forward]").click(function () {
				self.bring_forward();
			});
			object_ui.find("a[action=send_backward]").click(function () {
				self.send_backward();
			});
			object_ui.find("a[action=properties_object]").click(function () {
			...
			if (e.which == 3) {
				if (self.type == "square") {
					...
					if (self.properties.sx) {
						self.properties.sx = parseInt(self.properties.sx, 10);
					}

					if (self.properties.sy) {
						self.properties.sy = parseInt(self.properties.sy, 10);
					}

					if (self.properties.ex) {
						self.properties.ex = parseInt(self.properties.ex, 10);
					}

					if (self.properties.ey) {
						self.properties.ey = parseInt(self.properties.ey, 10);
					}
