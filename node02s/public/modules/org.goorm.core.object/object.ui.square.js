...
org.goorm.core.object.ui.square.prototype = {
	init: function (target, kind, proportion) {
		...
		$(target).find("canvas").mousedown(function (e) {
			...
			if (self.sx) {
				self.sx = parseInt(self.sx, 10);
			}

			if (self.sy) {
				self.sy = parseInt(self.sy, 10);
			}

			if (self.ex) {
				self.ex = parseInt(self.ex, 10);
			}

			if (self.ey) {
				self.ey = parseInt(self.ey, 10);
			}
			...
			if (!self.is_drawing_finished && self.is_dragging) {
				...
				//Dragging Top Left
					...
					if (self.proportion) {
						...
						self.sy = self.ey - parseInt(height, 10);
					} else {
						self.sx = x;
						self.sy = y;
					}
				...
				//Dragging Top
					...
					if (self.proportion) {
						...
						self.sx = middle - parseInt(width / 2, 10);
						self.ex = middle + parseInt(width / 2, 10);
					} else {
						self.sy = y;
					}
				}
				...
				//Dragging Right
					...
					if (self.proportion) {
						...
						self.ey = self.sy + parseInt(height, 10);
					} else {
						self.ex = x;
					}
				}
	...
	change_status: function (class_name) {
		...
		__target.removeClass("status_default")
			.removeClass("status_drawing_line")
			.removeClass("status_drawing_square")
			.removeClass("status_move")
			.removeClass("status_resize_top_left")
			.removeClass("status_resize_top_right")
			.removeClass("status_resize_bottom_left")
			.removeClass("status_resize_bottom_right")
			.removeClass("status_resize_top")
			.removeClass("status_resize_bottom")
			.removeClass("status_resize_left")
			.removeClass("status_resize_right");

		__target.addClass(class_name);
	},
